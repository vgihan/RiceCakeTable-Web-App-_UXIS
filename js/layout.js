$(function(){
	...
	$('.header .r_hcont .second .h_btn.p_people').click(function(event) {   //참가자 버튼
		var self = $(this);	
		if (self.hasClass("off")) {
		...
		if(shareSwitch) {
			if(socket.id === shareSocketId) {
				...
				if (roomType==="meeting")  document.getElementById('cont_inner').style="display: block;";
				return;
			}
	...
	$('.header .r_hcont .second .h_btn.share').click(function(event) {
		...
		if (self.hasClass("off")) {
			...
			if (roomType==="meeting")  document.getElementById('cont_inner').style="display: none;";
			self.removeClass("off").addClass("on");
		}
		if(shareSwitch) {
			if(socket.id === shareSocketId) {
