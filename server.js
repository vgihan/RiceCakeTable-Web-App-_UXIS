...
app.post("/send-file", function (request, response, next) {
  ...
  form.on("end", function () {
    for (var i = 0; i < this.openedFiles.length; i++) {
      ...
      var newpath =
        __dirname + "/uploads/" + roomid + "/" + this.openedFiles[i].name;
...
io.on("connection", function (socket) {
  ...
  socket.on("sender_offer", async (message) => {
    try {
      ...
      let pc = createReceiverPeerConnection(
        ...
        roomId,
        userName,
        ontrackHandler[message.purpose],
        message.purpose
      );
      ...
      if (message.purpose == "share") {
        //share가 목적인 경우 share화면의 stream의 오퍼를 받는것까진 성공했으니 switch를 바꿔줌
  ...
  socket.on("receiver_candidate", (message) => {
    try {
      ...
      if (
        !sendPCs[message.purpose][message.senderSocketId][
  ...
  socket.on("noCam", function (data) {
    ...
    socket.broadcast.to(data.roomId).emit("user_enter", {
      //노캠 사용자의 접속을 알림
      socketId: socket.id,
      roomId: data.roomId,
    ...
    if (message.text == "1 : 1 대화신청") {
      for (var key in roomList[message.roomId]) {
        if (key === message.target) {
          ...
          io.to(key).emit("other_ing_request", {
            // 나머지 사람에게
  ...
  socket.on("accept_1_1", (message) => {
    for (var key in roomList[message.roomId]) {
      if (key === message.target) {
        ...
        io.to(key).emit("other_accept_request", {
          // 나머지 사람에게
          user1Id: message.socketId,
          user2Id: message.target,
        });
  ...
  socket.on("refusal_1_1", (message) => {
    for (var key in roomList[message.roomId]) {
      if (key === message.target) {
        // 1:1 건 사람에게
        io.to(key).emit("refusal_request", {
          userName: message.userName,
          userId: message.socketId,
        });
      } else if (key !== message.socketId) {
        io.to(key).emit("other_end_request", {
          // 나머지 사람에게
          user1Id: message.socketId,
          user2Id: message.target,
        });
  ...
  socket.on("mute_list", (message) => {
    var others = [];
    for (var key in roomList[message.roomId]) {
      if (key !== message.target && key !== message.socketId) {
        // 대화 상대방에게
        others.push(key);
      }
    }
  ...
  socket.on("set_room_time", function (data) {
    ...
    else socket.emit("get_room_time", { time: roomToTime[data.roomId] });
  ...
  socket.on("reqUserInfo", function (data) {
    //보여줄 정보 전달
...
function shareJoinRoomHandler(message, socket) {
  //현재 화면공유중이라면 화면공유 정보를 넘김
  try {
    let rows = [];
